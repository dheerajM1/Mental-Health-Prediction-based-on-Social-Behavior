...
def vocablist():
    ...
    vocab = {term: freq for term, freq in vocab.items() if freq > 10} # Remove terms whose frequencies are less than a threshold (e.g., 10)
    ...
    tweets = []
    for line in open('unlabelled_tweets.txt').readlines():    #reading unlabelled tweet
        tweets.append(line)
    ...
    for text in tweets:
        ...
        for term in terms:
            if vocab.has_key(term):
                x[vocab[term]] += 1
        X.append(x)
    y = lr.predict(X)   #predicitng class label

    hello = map(str, y) # toconvert into string
    a = 0 #initialize the loop
    file = open("predicted_tweetslr.txt", "a+")   #new file opened
    for text in tweets:
        labelled_tweet= hello[a]+","+text #appending predicted class and tweet text
        file.write(labelled_tweet)   #writting the labelling tweet into file.
        a += 1
